    ...
    for(int x = skip; x < argc; x++) {
        if(*argv[x] == '-' && strlen(argv[x]) > 1) { /* options */
            ...
            if(opt == "o" || opt == "output") {
                ...
                fprintf(stderr, "-os2 is deprecated.\n");
                Option::host_mode = HOST_OS2_MODE;
                Option::target_mode = TARG_OS2_MODE;
                Option::target_mode_overridden = true;
            } else if(opt == "d") {
                Option::debug_level++;
...
static QStringList detectShellPath()
{
    /* @todo check if sh is actually the active shell of the process; relying on
     * the presence of sh.exe in PATH as done for Windows above is obviously not
     * enough */
    return QStringList();
}
#endif
...
#elif defined(Q_OS_OS2)
        Option::host_mode = Option::HOST_OS2_MODE;
...
{
    // this is in sync with initBinaryDir() from tools/linguist/lrelease/main.cpp
    QString ret;
#if defined(Q_OS_WIN)
    ...
    ret = QFileInfo(QString::fromLocal8Bit(appFileName)).filePath();
#else
