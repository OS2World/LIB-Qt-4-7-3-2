...
/*!
    ...
    \o \l os2Version() specifies the version of the OS/2
       operating system on which the application is run (OS/2 only).
    \endlist

    Some constants are defined only on certain platforms. You can use
    the preprocessor symbols Q_WS_WIN, Q_OS_MAC and Q_OS_OS2 to test that the
    application is compiled under Windows, Mac or OS/2.
...
/*!
    \fn QSysInfo::Os2Version QSysInfo::os2Version()
    \since 4.6

    Returns the version of the OS/2 operating system on which the
    application is run (OS/2 only).
*/
...
/*!
    \enum QSysInfo::Os2Version

    This enum provides symbolic names for the various versions of the
    OS/2 operating system. On OS/2, the
    QSysInfo::os2Version() function gives the version of the system on
    ...
    \value OV_ECS_2_1           eComStation 2.1 (either any of RCs or GA)
    \value OV_ECS_2_2           eComStation 2.2 (either any of RCs or GA)
    ...
    \sa WinVersion, SymbianVersion, MacVersion
*/
...
// Returns a version number of the component taken directly from its SYSLEVEL
// file (see http://www.edm2.com/index.php/Using_SYSLEVEL_Files_in_Your_Applications
// for more information about SYSLEVEL file internals)
// beginning from ecs 2.1 they changed the detection scheme once more
// the information is now in the ecs_inst.flg file as plain text
// the below function deals with both
enum FileType {
    SysLevel,
    eCSFlg };

static bool getSyslevelVersion(const QByteArray &fileName, FileType ft, char *version)
{
    bool success = false;
    int offSet;
    size_t bytesToR;
    ...
    if (!f)
        return success;
    // go to the offset of the data structure
    if ((success = fseek(f, offSet, SEEK_SET) == 0)) {
        // read in some bytes
        success = fread(version, 1, bytesToR, f) == bytesToR;
    }
    fclose(f);
    return success;
}

QSysInfo::Os2Version QSysInfo::os2Version()
{
    static QSysInfo::Os2Version os2ver = OV_Unknown;
    static bool os2VerSet = false;

    if (os2VerSet)
        return os2ver;
    os2VerSet = true;

    // detect the basic version number
    ULONG buf[3];
    if (DosQuerySysInfo(QSV_VERSION_MAJOR, QSV_VERSION_REVISION,
                        &buf, sizeof(buf)) != NO_ERROR)
        return os2ver;

    if (buf[0] != 20) // QSV_VERSION_MAJOR
        return os2ver;

    switch (buf[1]) { // QSV_VERSION_MINOR
        case 00: os2ver = OV_2_0; return os2ver;
        case 10: os2ver = OV_2_1; return os2ver;
        case 11: os2ver = OV_2_11; return os2ver;
        case 30: os2ver = OV_3_0; return os2ver;
        case 40: os2ver = OV_4_0; return os2ver;
        case 45: break;
        default: return os2ver;
    }
    ...
    // See http://ewiki.ecomstation.nl/HowToDetectVersion on how to detect
    // an eComStation version

    // read SYSLEVEL.OS2
    QByteArray fn = QByteArray("A:\\OS2\\INSTALL\\SYSLEVEL.OS2");
    fn.data()[0] += buf[0] - 1;
    char sys_os2_ver[2];
    if (!getSyslevelVersion(fn, SysLevel, sys_os2_ver))
        return os2ver;

    // read SYSLEVEL.ECS
    fn = QByteArray("A:\\OS2\\INSTALL\\SYSLEVEL.ECS");
    fn.data()[0] += buf[0] - 1;
    char sys_ecs_ver[2];
    if (!getSyslevelVersion(fn, SysLevel, sys_ecs_ver)) {
        // no or broken SYSLEVEL.ECS, it's either eCS 1.1 or below or non-eCS
        bool have_ecsreg = false;
        bool have_ecsreg11 = false;
        fn = QByteArray("A:\\OS2\\ECSREG.INI");
        fn.data()[0] += buf[0] - 1;
        have_ecsreg = access(fn, F_OK) == 0;
        fn = QByteArray("A:\\OS2\\ECSREG11.INI");
        fn.data()[0] += buf[0] - 1;
        have_ecsreg11 = access(fn, F_OK) == 0;
        // detect eCS 1.1
        if (have_ecsreg && have_ecsreg11)
            return os2ver = OV_ECS_1_1;
        // detect eCS 1.0
        if (have_ecsreg)
            return os2ver = OV_ECS_1_0;
        // detect WSeB (4.52)
        if (sys_os2_ver[0] == 0x45 && sys_os2_ver[1] == 0x02)
            return os2ver = OV_4_52;
        // fallback to Aurora (QSV_VERSION_MINOR is 45 here)
        return os2ver = OV_4_5;
    }

    // detect eCS >= 1.2
    if (sys_ecs_ver[0] == 0x12 && sys_ecs_ver[1] == 0x00)
        return os2ver = OV_ECS_1_2;
    if (sys_ecs_ver[0] >= 0x20 && sys_ecs_ver[1] == 0x00)
    {
        os2ver = OV_ECS_2_0;
        // read ecs_inst.flg
        char flg_ecs_ver[3];
        fn = QByteArray("A:\\ECS\\ECS_INST.FLG");
        fn.data()[0] += buf[0] - 1;
        if (getSyslevelVersion(fn, eCSFlg, flg_ecs_ver)) {
           if (flg_ecs_ver[0] == '2' && flg_ecs_ver[2] == '1')
               os2ver = OV_ECS_2_1;
           if (flg_ecs_ver[0] == '2' && flg_ecs_ver[2] == '2')
               os2ver = OV_ECS_2_2;
        }
        return os2ver;
    }

    return os2ver = OV_ECS_Unknown;
}

static QSysInfo::Os2Version os2verDummy = QSysInfo::os2Version();

#endif // ifdef Q_OS_OS2
...
void qt_message_output(QtMsgType msgType, const char *buf)
{
    ...
    if (msgType == QtFatalMsg
        ...
        abort(); // in kLIBC, it will INT 3 if LIBC_BREAKPOINT_ABORT is set and SIGABORT otherwise
#else
