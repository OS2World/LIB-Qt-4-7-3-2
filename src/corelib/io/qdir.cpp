...
{
#if defined(Q_OS_WIN) || defined(Q_OS_SYMBIAN) || defined(Q_OS_OS2)
    if (path.size() < 2)
        return QString();
...
QString QDir::absoluteFilePath(const QString &fileName) const
{
    ...
    // they may lead to different locations depending on where we are). This
    // requires special processing here. Don't be confused that everywhere else
    // in Qt (except QFSFileEngine::fileName() on OS/2) these paths will be
    // still considered absolute -- the latter is already so confusing and
    // inconsistent that we cannot make it any worse, while this function is
    // the only way to properly handle such paths w/o introducing custom
    // processing in each Qt application and we rely on it at least in
    // QLibraryInfo. See also isRelativePath() and isNotReallyAbsolutePath() in
    // qfsfileengine_os2.cpp.
    ...
    if (isAbsolutePath(fn)) {
        ...
        if (fn.length() > 1) ch1 = fn.at(1);
        if (fn.length() > 2) ch2 = fn.at(2);
        curDriveRelative = ch0 == QLatin1Char('/') && ch1 != QLatin1Char('/'); // "\ccc.dat"
        if (!curDriveRelative) {
            curDirOnDriveRelative = ch0.isLetter() && ch1 == QLatin1Char(':') && // "A:bbb.txt" (but not A:)
                                    !ch2.isNull() && ch2 != QLatin1Char('/');
            if (!curDirOnDriveRelative) {
                return fn; // really absolute ("A:\bla...", "\\server\share"), nothing to do
            }
        }
    }

    QString ret = d->fileEngine->fileName(QAbstractFileEngine::AbsoluteName);
    ...
    if (!fn.isEmpty()) {
        if (curDirOnDriveRelative) {
            ...
            if (curDriveRelative)
                ...
                ret += QLatin1Char('/'); // append the missing slash
            ret += fn;
        }
    }
    return ret;
...
#ifdef Q_OS_OS2
    // see QDir::absoluteFilePath() and QFSFileEngine::mkdir()
    QString fn = absoluteFilePath(dirName);
#else
...
#ifdef Q_OS_OS2
    // see QDir::absoluteFilePath() and QFSFileEngine::rmdir()
    QString fn = absoluteFilePath(dirName);
#else
    QString fn = filePath(dirName);
#endif
...
#ifdef Q_OS_OS2
    // see QDir::absoluteFilePath() and QFSFileEngine::mkdir()
    QString fn = absoluteFilePath(dirPath);
#else
...
QString QDir::cleanPath(const QString &path)
{
    ...
    QString drive = driveSpec(name);
    if (!drive.isEmpty())
        name = name.mid(drive.length());
...
#if (defined(Q_OS_WIN) && !defined(Q_OS_WINCE)) || defined(Q_OS_OS2) //allow unc paths
                if (i == 0 && drive.isEmpty())
                    break;
#endif
                i++;
            }
