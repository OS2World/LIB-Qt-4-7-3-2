        ...
        if(fileEngine()->copy(newName)) {
            ...
            if(!open(QFile::ReadOnly)) {
                ...
                if (error) {
                    ...
                    // ### note: if out is a QTemporaryFile instance, rename()
                    // below will fail on some platforms (such as OS/2) because
                    // the file will remain open for writing even after close()
...
/*!
    ...
    \bold{Note for the Windows and OS/2 Platforms}
    ...
    QIODevice::Text to \a mode. Predefined console devices, such as stdin and
    stdout, should be put to binary mode using \c _setmode() before passing them
    to this method:

    \snippet doc/src/snippets/code/src_corelib_io_qfile.cpp 5
...
/*!
    ...
    \a fd must be opened in binary mode (i.e., the open flags must contain
    _O_BINARY) when accessing files and other random-access devices. Qt will
    translate the end-of-line characters if you pass QIODevice::Text to
    \a mode. Predefined console devices, such as stdin and stdout, should be put
    to binary mode using \c _setmode() before passing them to this method:
