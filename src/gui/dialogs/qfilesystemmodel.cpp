...
QFileSystemModelPrivate::QFileSystemNode *QFileSystemModelPrivate::node(const QString &path, bool fetch) const
{
...
#if (!defined(Q_OS_WIN) || defined(Q_OS_WINCE)) && !defined(Q_OS_SYMBIAN) && !defined(Q_OS_OS2)
        && QDir::fromNativeSeparators(longPath) != QLatin1String("/")
#endif
        )
        return const_cast<QFileSystemModelPrivate::QFileSystemNode*>(&root);
    QModelIndex index = QModelIndex(); // start with "My Computer"
#if (defined(Q_OS_WIN) && !defined(Q_OS_WINCE)) || defined(Q_OS_OS2)
    ...
    for (int i = 0; i < pathElements.count(); ++i) {
        ...
        // On Windows and OS/2, "filename......." and "filename" are equivalent Task #133928
...
public:
                    ...
                    const QFileSystemModelPrivate::QFileSystemNode *r) const
    {
        ...
        case 0: {
            ...
            // place "." and ".." pseudo dirs on top
            if (left) {
                int lw = l->fileName == QLatin1String(".") ? 2 :
                         l->fileName == QLatin1String("..") ? 1 : 0;
                int rw = r->fileName == QLatin1String(".") ? 2 :
                         r->fileName == QLatin1String("..") ? 1 : 0;
                if (lw || rw)
                    return lw > rw;
            }
#endif
...
#if !defined(Q_OS_OS2) && (!defined(Q_OS_WIN) || defined(Q_OS_WINCE))
...
void QFileSystemModelPrivate::_q_directoryChanged(const QString &directory, const QStringList &files)
{
    ...
    QHash<FileNameKey, QFileSystemNode*>::const_iterator i = parentNode->children.constBegin();
...
#ifdef Q_OS_OS2
        // remove the invalid (non-existent) file unless it's a drive letter
        // (note that QFileInfoGatherer doesn't report invalid drive letters so
        // info.size() = -1 usually means that there is no media inserted etc.,
        // which is not a sufficient reason to hide the drive from the view)
        if (info.size() == -1 && !info.isSymLink() && parentNode != &root) {
#else
