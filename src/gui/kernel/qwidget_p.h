...
struct QTLWExtra {
    ...
    HWND fId;
    HSWITCH swEntry;
    HPOINTER iconPointer;
    QWidget *modalBlocker;
#endif
};
...
#elif defined(Q_WS_PM) // <----------------------------------------------------- PM
    QList<QWidget::PmEventFilter*> pmEventFilters;
#endif
};
...
public:
    ...
    inline QWExtra *extraData() const;
    inline QTLWExtra *topData() const;
    inline QTLWExtra *maybeTopData() const;
    inline QPainter *sharedPainter() const;
    inline bool pointInsideRectAndMask(const QPoint &) const;
    inline void setSharedPainter(QPainter *painter);
    inline QWidgetBackingStore *maybeBackingStore() const;
...
#elif defined(Q_WS_PM) // <----------------------------------------------------------- PM
    uint noPaintOnScreen : 1; // see qwidget_pm.cpp ::paintEngine()

    inline HWND frameWinId() const;
    HWND effectiveFrameWinId() const;
#ifdef QT_PM_NATIVEWIDGETMASK
    void validateObstacles();
#endif
    void reparentChildren();
    void setModalBlocker (QWidget *modal);
    QWidget *modalBlocker() const;
#elif defined(Q_WS_WIN) // <--------------------------------------------------------- WIN
...
inline HWND QWidgetPrivate::frameWinId() const
{
    return maybeTopData() && maybeTopData()->fId != NULLHANDLE ?
             maybeTopData()->fId : data.winid;
}

inline void QWidgetPrivate::setModalBlocker (QWidget *modal)
{
    topData()->modalBlocker = modal;
}

inline QWidget *QWidgetPrivate::modalBlocker() const
{
    QTLWExtra *top = maybeTopData();
    return top ? top->modalBlocker : 0;
}
