...
# elif defined(Q_OS_UNIX)
    ...
    return pair;
# elif defined(Q_OS_OS2)
    // note: the k* versions of libraries were in use once; support them for
    // backward compatibility
    static const char *ssl_dll[] = { "ssl10", "kssl10" };
    static const char *crypto_dll[] = { "crypto10", "kcrypt10" };

    QLibrary *libssl;
    for (size_t i = 0; i < sizeof(ssl_dll)/sizeof(ssl_dll[0]); ++i) {
        libssl = new QLibrary(QLatin1String(ssl_dll[i]));
        if (libssl->load())
            break;
        delete libssl;
        libssl = 0;
    ...
    QLibrary *libcrypto;
    for (size_t i = 0; i < sizeof(crypto_dll)/sizeof(crypto_dll[0]); ++i) {
        libcrypto = new QLibrary(QLatin1String(crypto_dll[i]));
        if (libcrypto->load())
            break;
        delete libcrypto;
        libcrypto = 0;
