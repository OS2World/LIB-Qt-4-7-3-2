...
MakefileGenerator::writePrlFile(QTextStream &t)
{
    ...
    if (Option::target_mode != Option::TARG_OS2_MODE) {
        // QMAKE_PRL_BUILD_DIR is not used on OS/2 so don't add it to avoid
        // possible confusion by exposing the full path to the build directory
    ...
    if(!project->isEmpty("TARGET_SHORT"))
        t << "QMAKE_PRL_TARGET_SHORT = " << project->first("TARGET_SHORT") << endl;
    ...
    if(project->isActiveConfig("staticlib") || project->isActiveConfig("explicitlib") ||
       !project->isEmpty("PRL_EXPORT_LIBS")) {
        t << "QMAKE_PRL_LIBS = ";
        if (!project->isEmpty("PRL_EXPORT_LIBS")) {
            // PRL_EXPORT_LIBS overrides anything else
            t << project->values("PRL_EXPORT_LIBS").join(" ");
        } else {
            QStringList libs;
            if(!project->isEmpty("QMAKE_INTERNAL_PRL_LIBS"))
...
{
    const QString del_suffix =
        Option::target_mode == Option::TARG_OS2_MODE ?
            QString(" >nul 2>&1"): // reduce noise
            QString::null;

    const QString inst_prefix =
        Option::target_mode == Option::TARG_OS2_MODE ?
            QString::null: // report errors (copy command overwrites quietly)
            QString("-");

    QString rm_dir_contents("-$(DEL_FILE)");
    if (!isDosLikeShell()) //ick
    ...
    for(QStringList::Iterator it = l.begin(); it != l.end(); ++it) {
        ...
        if(dst.right(Option::dir_sep.length()) != Option::dir_sep)
            dst += Option::dir_sep;
        ...
        //masks
        tmp = project->values((*it) + ".files");
        if(!installConfigValues.contains("no_vpath"))
            tmp = findFilesInVPATH(tmp, VPATH_NoFixify);
        if(!installConfigValues.contains("no_fixify"))
        ...
        if(!tmp.isEmpty()) {
            ...
            for(QStringList::Iterator wild_it = tmp.begin(); wild_it != tmp.end(); ++wild_it) {
                ...
                if(is_target || exists(wild)) { //real file or target
                    ...
                    QString cmd;
                    if (fi.isDir())
                       cmd = inst_prefix + "$(INSTALL_DIR)";
                    else if (is_target || fi.isExecutable())
                       cmd = inst_prefix + "$(INSTALL_PROGRAM)";
                    else
                    ...
                    if(!uninst.isEmpty())
                        uninst.append("\n\t");
                    uninst.append(rm_dir_contents + " " + filePrefixRoot(root, fileFixify(dst + filestr, FileFixifyAbsolute, false)) + del_suffix);
                    continue;
                }
                ...
                if (installConfigValues.contains("no_check_exist") && files.isEmpty()) {
                    ...
                    if (installConfigValues.contains("directory")) {
                        cmd = inst_prefix + QLatin1String("$(INSTALL_DIR)");
                        if (!dst_file.endsWith(Option::dir_sep))
                            dst_file += Option::dir_sep;
                        dst_file += fi.fileName();
                    } else if (installConfigValues.contains("executable")) {
                        cmd = inst_prefix + QLatin1String("$(INSTALL_PROGRAM)");
                    } else if (installConfigValues.contains("data")) {
                        cmd = inst_prefix + QLatin1String("$(INSTALL_FILE)");
                    } else {
                        cmd = inst_prefix + QString(fi.isExecutable() ? "$(INSTALL_PROGRAM)" : "$(INSTALL_FILE)");
                    }
                    cmd += " " + wild + " " + dst_file + "\n";
                    target += cmd;
                    if(!uninst.isEmpty())
                ...
                for(int x = 0; x < files.count(); x++) {
                    ...
                    if(!uninst.isEmpty())
                        uninst.append("\n\t");
                    uninst.append(rm_dir_contents + " " + filePrefixRoot(root, fileFixify(dst + file, FileFixifyAbsolute, false)) + del_suffix);
                    ...
                    QString cmd = inst_prefix + QString(fi.isDir() ? "$(INSTALL_DIR)" : "$(INSTALL_FILE)") + " " +
        ...
        if(!target.isEmpty() || installConfigValues.indexOf("dummy_install") != -1) {
            ...
            for(QStringList::ConstIterator pit = dirs.begin(); pit != dirs.end(); ++pit) {
                ...
                if (!isDosLikeShell() && !tmp_dst.endsWith(Option::dir_sep))
            ...
            if(!uninst.isEmpty()) {
                ...
                t << "\n\t"
                  << uninst.join(" ") << "\n\t"
                  << "-$(DEL_DIR) " << filePrefixRoot(root, dst) << " " << del_suffix << endl << endl;
            }
            t << endl;
            ...
            if(!wrote_clean_cmds || !wrote_clean) {
                ...
                if(!wrote_clean) {
                    if(project->isActiveConfig("no_delete_multiple_files")) {
                        ...
                        QString files, file;
                        const int commandlineLimit =
                            Option::target_mode == Option::TARG_OS2_MODE ?
                                1000: // OS/2 CMD.EXE limit (1024 - suffix - reserve)
                                2047; // NT limit, expanded
...
QString MakefileGenerator::buildArgs(const QString &outdir)
{
    ...
    if (Option::target_mode_overridden) {
        if (Option::target_mode == Option::TARG_MACX_MODE)
            ...
            ret += " -win32";
        else if(Option::target_mode == Option::TARG_OS2_MODE)
            ret += " -os2";
    }
...
#define MAKE_CD_IN_AND_OUT(directory) \
        ...
        if(!subtarget->profile.isEmpty()) {
            ...
            if(out.startsWith(out_directory))
                out = out.mid(out_directory.length());
            t << mkfile << ": " << "\n\t";
            if(!in_directory.isEmpty()) {
    ...
    for(int s = 0; s < targetSuffixes.size(); ++s) {
        ...
        const QString del_suffix =
            Option::target_mode == Option::TARG_OS2_MODE ?
                ...
                QString::null;
        if(suffix == "clean") {
            t << varGlue("QMAKE_CLEAN","\t-$(DEL_FILE) ",del_suffix+"\n\t-$(DEL_FILE) ", del_suffix) << endl;
        } else if(suffix == "distclean") {
            QString ofile = Option::fixPathToTargetOS(fileFixify(Option::output.fileName()));
            if(!ofile.isEmpty())
                t << "\t-$(DEL_FILE) " << ofile << del_suffix << endl;
    ...
    for(QStringList::Iterator qut_it = qut.begin(); qut_it != qut.end(); ++qut_it) {
        ...
        if(project->values((*qut_it) + ".CONFIG").indexOf("recursive") != -1) {
            ...
            for(int target = 0; target < targets.size(); ++target) {
                ...
                // note that we always pass the makefile as argument since it's
                // hard to tell if it matches the platform make's default
                // file name or not (and it can be also specified indirectly
                // through $(MAKEFILE))
                QString makefilein = " -f " + subtarget->makefile;
                              ...
                              FileFixifyType fix, bool canon) const
{
    ...
    Qt::CaseSensitivity cs = QFile(file).fileEngine()->caseSensitive() ?
    ...
    if(fix == FileFixifyAbsolute || (fix == FileFixifyDefault && project->isActiveConfig("no_fixpath"))) {
        ...
        if(out_dir != in_dir || !qfileinfo.isRelative()) {
            ...
            if(ret == match_dir) {
                ret = "";
            } else if(ret.startsWith(match_dir + Option::dir_sep, cs)) {
                ...
                for(int i = 1; i <= depth; i++) {
                    ...
                    if(match_dir.isEmpty())
                        break;
                    if(ret.startsWith(match_dir + Option::dir_sep, cs)) {
                        //concat
