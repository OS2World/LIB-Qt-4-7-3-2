...
void Win32MakefileGenerator::writeCleanParts(QTextStream &t)
{
    ...
    t << "\n\t" << del_statement << " $(DESTDIR_TARGET)" << del_suffix << endl;
    {
        ...
        if(!ofile.isEmpty())
            t << "\t" << del_statement << " " << ofile << del_suffix << endl;
    }
    t << endl;
}
...
void Win32MakefileGenerator::writeStandardParts(QTextStream &t)
{
    ...
    QString target = project->first("TARGET_SHORT");
    if (target.isEmpty())
        target = project->first("TARGET");
    target += project->first("TARGET_EXT");
    target.remove("\"");
    project->values("DEST_TARGET").prepend(destDir + target);

    writeObjectsPart(t);
    writeRcAndDefVariables(t);
    ...
    t << "DESTDIR        = " << escapeFilePath(destDir) << endl;
...
void Win32MakefileGenerator::writeRcAndDefVariables(QTextStream &t)
{
    t << "DEF_FILE      = " << varList("DEF_FILE") << endl;
    t << "RES_FILE      = " << varList("RES_FILE") << endl; // Not on mingw, can't see why not though...
}

void Win32MakefileGenerator::writeRcAndDefPart(QTextStream &t)
{
    if(!project->values("RC_FILE").isEmpty()) {
    ...
    if(t == "target" && project->first("TEMPLATE") == "lib") {
        if(project->isActiveConfig("create_prl") && !project->isActiveConfig("no_install_prl") &&
           !project->isEmpty("QMAKE_INTERNAL_PRL_FILE")) {
            ...
            ret += inst_prefix + "$(INSTALL_FILE) \"" + project->first("QMAKE_INTERNAL_PRL_FILE") + "\" \"" + dst_prl + "\"";
            if(!uninst.isEmpty())
                uninst.append("\n\t");
            uninst.append("-$(DEL_FILE) \"" + dst_prl + "\"" + del_suffix);
        }
        if(project->isActiveConfig("shared") && !project->isActiveConfig("plugin")) {
            ...
            if(!ret.isEmpty())
                ret += "\n\t";
            ret += QString(inst_prefix + "$(INSTALL_FILE)") + " \"" + src_targ + "\" \"" + dst_targ + "\"";
            if(!uninst.isEmpty())
                uninst.append("\n\t");
            uninst.append("-$(DEL_FILE) \"" + dst_targ + "\"" + del_suffix);
        }
        if(project->isActiveConfig("shared") && project->isActiveConfig("debug")) {
            QString pdb_target = getPdbTarget();
            if (!pdb_target.isEmpty()) {
                pdb_target.remove('"');
...
QString Win32MakefileGenerator::escapeFilePath(const QString &path) const
{
    ...
    if(!ret.isEmpty()) {
        ...
        // Note: also quote paths ending with the backslash to avoid
        // interpreting it as a linebreak
        if(ret.contains(' ') || ret.endsWith('\\'))
            ret = "\"" + ret + "\"";
