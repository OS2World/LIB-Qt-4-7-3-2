...
enum ExpandFunc { E_MEMBER=1, E_FIRST, E_LAST, E_CAT, E_FROMFILE, E_EVAL, E_LIST,
                  ...
                  E_SIZE, E_GENERATE_UID, E_FIXPATH_TARGET, E_FIXPATH_LOCAL };
QMap<QString, ExpandFunc> qmake_expandFunctions()
{
    ...
    if(!qmake_expand_functions) {
        ...
        qmake_expand_functions->insert("fixpath_target", E_FIXPATH_TARGET);
        qmake_expand_functions->insert("fixpath_local", E_FIXPATH_LOCAL);
    }
...
QStringList qmake_feature_paths(QMakeProperty *prop=0)
{
    QStringList concat;
    {
        ...
        case Option::TARG_SYMBIAN_MODE:
            ...
            break;
        case Option::TARG_OS2_MODE:
            concat << base_concat + QDir::separator() + "os2";
            break;
        }
        concat << base_concat;
    }
...
QMakeProject::isActiveConfig(const QString &x, bool regex, QMap<QString, QStringList> *place)
{
    ...
    else if(Option::target_mode == Option::TARG_OS2_MODE && x == "os2")
        return true;
    ...
    if((regex && re.exactMatch(spec)) || (!regex && spec == x))
        ...
        if (!spec_org.isEmpty()) {
            spec = QFileInfo(spec_org.at(0)).fileName();
            if((regex && re.exactMatch(spec)) || (!regex && spec == x))
                return true;
        }
    }
                              ...
                              QMap<QString, QStringList> &place)
{
    ...
    if(replaceFunctions.contains(func)) {
        ...
        FunctionBlock *popped = function_blocks.pop();
        Q_ASSERT(popped == defined);
        Q_UNUSED(popped);
        return ret;
    }
    ...
    case E_GENERATE_UID:
        ...
        break;
    case E_FIXPATH_TARGET:
    case E_FIXPATH_LOCAL:
        if (args.count() < 1 || args.count() > 3) {
            fprintf(stderr, "%s:%d: fixpath_%s(str) requires one argument.\n",
                    parser.file.toLatin1().constData(), parser.line_no,
                    func_t == E_FIXPATH_TARGET ? "target" : "local");
        } else {
            bool fix_env = true;
            if(args.count() > 1)
                fix_env = (args[1].toLower() == "true" || args[1].toInt());
            bool canonical = true;
            if(args.count() > 2)
                canonical = (args[2].toLower() == "true" || args[1].toInt());
            if(func_t == E_FIXPATH_TARGET)
                ret += Option::fixPathToTargetOS(args[0], fix_env, canonical);
            else
                ret += Option::fixPathToLocalOS(args[0], fix_env, canonical);
        }
        break;
    default: {
...
QStringList &QMakeProject::values(const QString &_var, QMap<QString, QStringList> &place)
{
    ...
    if(var == QLatin1String("LITERAL_WHITESPACE")) { //a real space in a token)
        ...
        if(type == "os") {
            ...
            ret = QString::fromLocal8Bit(qgetenv("HOSTNAME"));
        } else if(type == "version" || type == "version_string") {
            ULONG buf [3];
            DosQuerySysInfo(QSV_VERSION_MAJOR, QSV_VERSION_REVISION,
                            &buf, sizeof(buf));
            if(type == "version")
                ret = QString().sprintf("%lu.%lu.%lu", buf[0], buf[1], buf[2]);
            else {
                /* Warp 3 is reported as 20.30 */
                /* Warp 4 is reported as 20.40 */
                /* Aurora and eCS are reported as 20.45 */
                if (buf[0] == 20 && buf[1] == 30)
                    ret = "Warp3";
                else if (buf[0] == 20 && buf[1] == 40)
                    ret = "Warp4";
                else if (buf[0] == 20 && buf[1] == 45) {
                    if (QString::fromLocal8Bit(qgetenv("OS")) == "ecs")
                        ret = "eComStation";
                    else
                        ret = "Aurora";
                }
