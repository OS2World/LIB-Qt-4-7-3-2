...
mutexGuard::~mutexGuard()
{
...
#elif defined(_CL_HAVE_OS2_THREADS)
	#include "CLucene/config/threadOS2.h"

	mutex_os2::mutex_os2(const mutex_os2& clone)
	{
		DosCreateMutexSem(NULL, &mtx, 0, FALSE);
	}
	...
	void mutex_os2::unlock()
	{
		DosReleaseMutexSem(mtx);
	}

	ULONG mutex_os2::getCurrentTid(void)
	{
		PTIB pTIB;
		return (DosGetInfoBlocks(&pTIB, NULL) == 0) ?
