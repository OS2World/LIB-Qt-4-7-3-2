...
NEVER_INLINE CollectorBlock* Heap::allocateBlock()
{
    ...
    if (!address)
        CRASH();
#elif (OS(OS2) && !HAVE(POSIX_MEMALIGN))
    void* address = NULL;
    APIRET arc = DosAllocMem(&address, BLOCK_SIZE, PAG_COMMIT | PAG_READ | PAG_WRITE | OBJ_ANY);
    if (arc != NO_ERROR) {
        // try low mem if high mem (OBJ_ANY) fails
        arc = DosAllocMem(&address, BLOCK_SIZE, PAG_COMMIT | PAG_READ | PAG_WRITE);
        if (arc != NO_ERROR)
            CRASH();
    }
...
static inline void* currentThreadStackBase()
{
    ...
    return static_cast<void*>(ptib->tib_pstacklimit);
