...
static const char *qt_module_path()
{
    static char path[CCHMAXPATH] = {'\0'};
    if (!path[0]) {
        HMODULE hmod;
        ULONG objNum, offset;
        DosQueryModFromEIP(&hmod, &objNum, sizeof(path), path, &offset,
                           (ULONG)qt_module_path);
        DosQueryModuleName(hmod, sizeof(path), path);
...
// expands environment variables in the form $(ENVVAR)
static QString expandEnvVars(const QString &str)
{
    ...
    while((rep = reg_var.indexIn(ret)) != -1) {
        ret.replace(rep, reg_var.matchedLength(),
                    QFile::decodeName(qgetenv(ret.mid(rep + 2,
...
#ifdef Q_OS_OS2
    if (!QFile::exists(qtconfig)) {
        // search in the directory that contains the DLL or EXE where this
        // code is located (e.g. QtCore4.dll or app.exe if Qt is a static lib)
        qtconfig = QString::fromLocal8Bit(qt_module_path());
        qtconfig = QDir::fromNativeSeparators(qtconfig) + QLatin1String("/qt.conf");
        qtconfig = QDir::cleanPath(qtconfig);
    }
#endif
#endif
    if (QFile::exists(qtconfig))
...
QLibraryInfo::location(LibraryLocation loc)
{
...
#ifdef Q_OS_OS2
    // QDir::isRelativePath() isn't precise on OS/2 (see QDir::absoluteFilePath())
    // therefore we don't rely on it
    if (loc == PrefixPath) {
#ifdef BOOTSTRAPPING
        QFileInfo fi(qmake_libraryInfoFile());
        ret = QDir::cleanPath(QDir(fi.absolutePath()).absoluteFilePath(ret));
#else
        QSettings *config = QLibraryInfoPrivate::configuration();
        if (config) {
            // if we read paths from qt.conf, we make the prefix path absolute
            // to this file's directory
            QFileInfo fi(config->fileName());
            ret = QDir::cleanPath(QDir(fi.absolutePath()).absoluteFilePath(ret));
        } else {
            // otherwise we make the prefix path absolute to the DLL/EXE directory
            QString path = QString::fromLocal8Bit(qt_module_path());
            ret = QDir::cleanPath(QDir(path).absoluteFilePath(ret));
        }
#endif
    } else {
        // we make any other path absolute to the prefix directory
        ret = QDir::cleanPath(QDir(location(PrefixPath)).absoluteFilePath(ret));
    }
    ...
    if (QDir::isRelativePath(ret)) {
        if (loc == PrefixPath) {
...
#ifdef BOOTSTRAPPING
            QFileInfo fi(qmake_libraryInfoFile());
            return QDir::cleanPath(QDir(fi.absolutePath()).absoluteFilePath(ret));
#else
...
#endif
                return QDir::cleanPath(QDir(QCoreApplication::applicationDirPath()).absoluteFilePath(ret));
            } else {
            ...
            // we make any other path absolute to the prefix directory
            return QDir::cleanPath(QDir(location(PrefixPath)).absoluteFilePath(ret));
        }
    }
