...
{
    enum {
        PollingInterval = 3000, // time to wait after checking all watched items, ms
    };
...
QPollingFileSystemWatcherEngine::QPollingFileSystemWatcherEngine()
    : askedToFinish(false), startOver(false)
{
#ifndef QT_NO_THREAD
    moveToThread(this);
#endif
...
void QPollingFileSystemWatcherEngine::run()
{
    ...
    forever {
        ...
        if (!startOver && !askedToFinish) {
            waitCond.wait(&mutex, PollingInterval);
        }
        if (askedToFinish) {
            askedToFinish = false;
            break;
...
#elif defined(Q_OS_OS2)
    QOS2FileSystemWatcherEngine *engine = new QOS2FileSystemWatcherEngine();
    if (engine->isOk())
        return engine;
    delete engine;
    return 0;
