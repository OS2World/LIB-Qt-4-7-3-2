...
QString QFSFileEnginePrivate::canonicalized(const QString &path)
{
    ...
    if (path.size() == 1 && path.at(0) == QLatin1Char('/'))
        return path;
#endif
#if defined(Q_OS_LINUX) || defined(Q_OS_SYMBIAN) || defined(Q_OS_MAC) || defined(Q_OS_OS2)
...
/*! \fn QString QFSFileEngine::currentPath(const QString &fileName)
  ...
  For Windows and OS/2, returns the canonicalized form of the current path used
  by the file engine for the drive specified by \a fileName.  On Windows and on
  OS/2, each drive has its own current directory, so a different path is
  returned for file names that include different drive names (e.g. A: or C:).
