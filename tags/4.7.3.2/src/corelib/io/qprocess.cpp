...
/*!
    ...
    \section1 Notes for Windows and OS/2 Users

    Some Windows and OS/2 commands (for example, \c dir) are not provided
    by separate applications, but by the command interpreter itself.
...
/*!
    ...
    \bold{OS/2:} The started process will run in its own session which is
    completely independent from the session of the starting application and
    will continue execution after the starting application terminates.
    ...
    Note that on OS/2, the PID of the started process is unknown and the value
    returned in \a pid is always 0.
*/
