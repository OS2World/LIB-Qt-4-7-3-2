    ...
    */
    ...
    if (pathHash->isEmpty()) {
       ...
       */
        ...
        /* Though this code is very similar to the following #else block, the
         * definition of the "absolute path" is different on OS/2 (see
         * isRelativePath() in qfsfileengine_os2.cpp) so take it into account)*/
        QString userPath;
        QString env = QFile::decodeName(qgetenv("XDG_CONFIG_HOME"));
        if (env.isEmpty()) {
            userPath = homePath;
            userPath += QDir::separator();
            userPath += QLatin1String(".config");
        } else {
            userPath = QDir::cleanPath(QDir(homePath).absoluteFilePath(env));
        }
        userPath = QDir::cleanPath(userPath) + QLatin1Char('/');
...
/*!
    ...
    information is often stored in the system registry on Windows and OS/2,
    ...
    Setting keys can contain any Unicode characters. The Windows and OS/2
    ...
    On Windows and OS/2, NativeFormat settings are stored in the following
    registry paths:
    ...
    On OS/2, the following files are used:
    ...
    setPath(). On Unix, Mac OS X and OS/2, the user can override them by setting
    the \c XDG_CONFIG_HOME environment variable; see setPath() for details.
    ...
    \section2 Accessing the Windows and OS/2 Registry Directly

    On Windows and OS/2, QSettings lets you access settings that have been
    ...
    \section2 Accessing Common Registry Settings on Windows and OS/2

    On Windows and OS/2, it is possible for a key to have both a value and
    subkeys. Its default value is accessed by using "Default" or "." in
    place of a subkey:

    \snippet doc/src/snippets/code/src_corelib_io_qsettings.cpp 6

    On other platforms than Windows and OS/2, "Default" and "." would be
    ...
    \list
    \o  The Windows and OS/2 system registry has the following limitations: A
...
/*! \enum QSettings::Format
    ...
    \value NativeFormat  Store the settings using the most
                         ...
                         On Windows and OS/2, this means the system registry;
...
/*!
    ...
    \c .plist file. On Windows and OS/2, \a fileName is a path in the system
    registry.
...
/*!
    ...
    On Windows and OS/2, if the format is QSettings::NativeFormat, the return
...
/*!
  ...
  Note that the Windows and OS/2 registry and INI files use case-insensitive
...
/*!
    ...
    \row                                                        \o SystemScope \o \c /etc/xdg
    \row    \o{1,2} OS/2        \o{1,2} IniFormat               \o UserScope   \o \c %HOME%\.config
    \row                                                        \o SystemScope \o \c %ETC%\xdg
    \endtable

    The default UserScope paths on Unix, Mac OS X and OS/2 (\c $HOME/.config or
    $HOME/Settings) can be overridden by the user by setting the
    \c XDG_CONFIG_HOME environment variable. The default SystemScope
    paths on Unix, Mac OS X (\c /etc/xdg) and OS/2 (\c %ETC%\xdg) can be
    overridden when building the Qt library using the \c configure script's
    \c
    --sysconfdir flag (see QLibraryInfo for details).

    Setting the NativeFormat paths on Windows, Mac OS X and OS/2 has no effect.
...
/*! \enum QSettings::System
    ...
    \value Windows Microsoft Windows and IBM OS/2 systems
