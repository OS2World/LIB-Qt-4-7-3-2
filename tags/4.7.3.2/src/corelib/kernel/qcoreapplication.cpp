...
#if defined(Q_WS_WIN) || defined(Q_WS_MAC) || defined(Q_WS_PM)
extern QString qAppFileName();
#endif

#if !defined(Q_OS_WIN) && !defined(Q_WS_PM)
#ifdef Q_OS_MAC
...
void QCoreApplication::init()
{
    ...
    // QCoreApplication is most likely constructed after static QFactroyLoader
    // instances (such as the one for QTextCodecFactoryInterface_iid). Hence, we
    // need to inform these instances that we have added new paths.
    QFactoryLoader::refreshAll();
#ifndef QT_NO_TEXTCODECPLUGIN
    // libaryPaths() calls QTextCodec::codecForLocale() before it adds the
    // appiication path to the library path. As a result, if this path contains
    // a plugin for the codec requested in LANG, this request will be ignored
    // because the locale would have been already set by codecForLocale()
    // Request codec auto-detection again.
    extern void qt_resetCodecForLocale(); // qtextcodec.cpp
    qt_resetCodecForLocale();
#endif
#endif
...
QString QCoreApplication::applicationFilePath()
{
    ...
    QString appFileName = qAppFileName();
    if(!appFileName.isEmpty()) {
        QFileInfo fi(appFileName);
        d->cachedApplicationFilePath = fi.canonicalFilePath();
    }
    return d->cachedApplicationFilePath;
...
#elif defined(Q_WS_PM)
    return pmEventFilter(reinterpret_cast<QMSG *>(message), reinterpret_cast<MRESULT *>(result));
#else
    return false;
#endif
}
