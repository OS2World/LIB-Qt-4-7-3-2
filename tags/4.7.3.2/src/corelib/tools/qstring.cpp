...
#if !defined(Q_WS_MAC) && defined(Q_OS_UNIX) && !defined(Q_OS_OS2)
                                       ...
                                       const QChar *data2, int length2)
{
    ...
    extern Q_CORE_EXPORT QString qt_system_locale_name(); // qlocale.cpp
    // UniCreateLocaleObject("") creates an "en_US" locale when LANG is not set
    // but we want to get the system locale so always use the exact name as
    // returned by qt_system_locale_name() (it also takes LANG into account)
#if !defined(QT_NO_THREAD)
    static QAtomicPointer<void> localeObj;
    if (!localeObj) {
        LocaleObject lo = 0;
        UniCreateLocaleObject(UNI_UCS_STRING_POINTER,
                              qt_system_locale_name().utf16(), &lo);
        if (!localeObj.testAndSetRelaxed(0, lo))
            UniFreeLocaleObject(lo); // we are too late
    }
#else
    static LocaleObject localeObj = 0;
    if (!localeObj)
        UniCreateLocaleObject(UNI_UCS_STRING_POINTER,
                              qt_system_locale_name().utf16(), &localeObj);
#endif
    if (localeObj) {
        // note: we assume that data1 and data2 are zero-terminated (to avoid
        // creation of dumb deep copies) which is at least true for <= 4.7.x.
#if QT_VERSION <= 0x040703
        return UniStrcoll(localeObj, (UniChar*)data1, (UniChar *)data2);
#else
# error "Check me!"
#endif
    } else {
