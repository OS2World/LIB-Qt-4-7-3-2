...
public:
    ...
    // On Windows and OS/2, file names are case insensitive so use a lowercased
    // string as a key in the hash map of name->node pairs. Note that strictly
    // speaking we should use QAbstractFileEngine::caseSensitive() to check if
    // the case should matter but on these platforms this method in QFSFileEngine
    // always returns false while doing this check would require to add a
    // QFileSystemNode* argument to the constructor and hence change almost every
    // line that uses the hash because the automatic QString->FileNameKey would
    // not work in this case. Note2: on Windows, evreything actually works without
    // this class but that's just because we use qt_GetLongPathName() which
    // always returns a real filename case and any changes in the case are
    // instantly picked up by QFileSystemWatcher anyway (otherwise we'd get dups
    // in the file ilst there too).
    class FileNameKey : public QString
    {
    public:
        FileNameKey(const QString &copy) : QString(copy.toLower()) {}
    };
#else
    typedef QString FileNameKey;
#endif
    ...
    public:
        ...
        ~QFileSystemNode() {
            QHash<FileNameKey, QFileSystemNode*>::const_iterator i = children.constBegin();
        ...
        void updateIcon(QFileIconProvider *iconProvider, const QString &path) {
            ...
            QHash<FileNameKey, QFileSystemNode *>::const_iterator iterator;
        ...
        bool isVisible;
        QHash<FileNameKey, QFileSystemNode *> children;
