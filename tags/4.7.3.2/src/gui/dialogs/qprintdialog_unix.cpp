...
#if !defined(QT_NO_CUPS) && (!defined(QT_NO_LIBRARY) || defined (Q_WS_PM))
...
#if defined (Q_OS_UNIX) || defined (Q_WS_PM)
...
static QString defaultOutputFileName(const QString &docName, QPrinter::OutputFormat format)
{
    if (format == QPrinter::PostScriptFormat || format == QPrinter::PdfFormat) {
        ...
        if (cur.left(home.length()) != home)
            cur = home;
#endif
#if defined(Q_WS_X11) || defined(Q_WS_PM)
        if (docName.isEmpty()) {
...
void QUnixPrintWidgetPrivate::_q_printerChanged(int index)
{
    ...
    if (filePrintersAdded) {
        ...
        if (index > printerCount - 3) { // PDF or postscript
            ...
            if (filename.isEmpty())
                filename = defaultOutputFileName(printer->docName(), pdfPrinter ? QPrinter::PdfFormat :
                                                 QPrinter::PostScriptFormat);
            else {
                QFileInfo fileInfo = QFileInfo(filename);
                QString suffix = fileInfo.suffix();
                if (pdfPrinter && suffix == QLatin1String("ps"))
                    filename = fileInfo.path() + QDir::separator() +
                        fileInfo.completeBaseName() + QLatin1String(".pdf");
                if (!pdfPrinter && suffix == QLatin1String("pdf"))
                    filename = fileInfo.path() + QDir::separator() +
                        fileInfo.completeBaseName() + QLatin1String(".ps");
                filename = QDir::cleanPath(filename);
            }
            widget.filename->setText(QDir::toNativeSeparators(filename));
            widget.lOutput->setEnabled(true);
            widget.fileBrowser->setEnabled(true);
            if (propertiesDialog)
...
void QUnixPrintWidgetPrivate::applyPrinterProperties(QPrinter *p)
{
    ...
    if (p->outputFileName().isEmpty())
        widget.filename->setText(QDir::toNativeSeparators(
            defaultOutputFileName(p->docName(), p->outputFormat())));
    else
        widget.filename->setText(QDir::toNativeSeparators(p->outputFileName()));
