...
QSize QSideBarDelegate::sizeHint (const QStyleOptionViewItem &option,
                                  const QModelIndex &index) const
{
    // Make the item height in the side bar little bit bigger than in the
    // file list view (to emphasize it). Note that this is better than
    // previously used enforcing the icon size on the list with setIconSize()
    // because it doesn't cause icon scaling (whigh results in blurred images)
    QSize size = QStyledItemDelegate::sizeHint(option, index);
    int height = size.height();
    size.setHeight(height + height / 4);
    return size;
}
