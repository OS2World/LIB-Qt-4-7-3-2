...
#if defined(Q_WS_PM)
    HBITMAP toPmHBITMAP(HBITMAP *mask = 0, bool embedRealAlpha = false) const;
    static QPixmap fromPmHBITMAP(HBITMAP hbm, HBITMAP hbmMask = NULLHANDLE);
    static HPOINTER toPmHPOINTER(const QIcon &icon, bool isPointer = false,
                                 int hotX = 0, int hotY = 0,
                                 bool embedRealAlpha = false, bool isMini = true);
    static QIcon fromPmHPOINTER(HPOINTER hpointer, QPixmap *pixmap = 0,
                                QPixmap *pixmapMini = 0);
#endif
