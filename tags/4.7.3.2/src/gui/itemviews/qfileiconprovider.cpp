...
#elif defined(Q_WS_PM)
    QIcon getPmIcon(const QFileInfo &fi) const;
#elif defined(Q_WS_MAC)
...
#elif defined(Q_WS_PM)
QIcon QFileIconProviderPrivate::getPmIcon(const QFileInfo &fileInfo) const
{
    QIcon retIcon;

    if (fileInfo.isRoot()) {
        // Unfortunately, WinLoadFileIcon() returns a regular folder icon for
        // paths like "C:\" (and nothing for "C:") instead of a drive icon.
        // Getting the latter involves calling WPS object methods so we leave it
        // out for now and let the stock Qt drive-specific icons be used instead.
        return retIcon;
    }

    QByteArray path = QFile::encodeName(QDir::toNativeSeparators(
        QDir::cleanPath(fileInfo.canonicalFilePath())));
    HPOINTER hicon = WinLoadFileIcon(path, FALSE);
    if (hicon == NULLHANDLE) {
        // WinLoadFileIcon() fails on non-existing paths but we still want a
        // system icon for it to be "in tune" with other icons. One known case
        // is a directory on an NDFS SMB multi-resource share that links to a
        // remote CD-ROM drive with no disk in it. This actually represents a
        // more common problem when an icon for a non-existent path is
        // requested. We will solve it by providing an icon of a known-to-exist
        // path element instead. In order to differentiate between a
        // non-existent folder and file we use a return code from
        // DosQueryPathInfo() which appears to be ERROR_PATH_NOT_FOUND for the
        // SMB multi-resource case and ERROR_FILE_NOT_FOUND otherwise. Note that
        // a calling app may actually already know if it's a file or a directory
        // from the listing of a parent directory (or we could do such a listing
        // here) but it looks like too much hassle to use this approach here.
        FILESTATUS3 info;
        APIRET arc = DosQueryPathInfo(path, FIL_STANDARD, &info, sizeof(info));
        if (arc == ERROR_PATH_NOT_FOUND) {
            // a known-to-exist directory
            hicon = WinLoadFileIcon("\\", FALSE);
        } else {
            // a known-to-exist file
            ...
            hicon = WinLoadFileIcon(kernelFilePath, FALSE);
        }
    }
    if (hicon != NULLHANDLE) {
        // we're requesting the system (shared) icon handle which should be
        // always the same for the given file until the icon is changed, so use
        // the bitmap handles as a key in the pixmap cache
        QString key = QString(QLatin1String("qt_hicon_%1")).arg(hicon);
        QString keyMini = key + QLatin1String("_m");
        QPixmap pixmap;
        QPixmapCache::find(key, pixmap);
        if (!pixmap.isNull()) {
            retIcon.addPixmap(pixmap);
            QPixmapCache::find(keyMini, pixmap);
            if (!pixmap.isNull())
                retIcon.addPixmap(pixmap);
        } else {
            QPixmap mini;
            retIcon = QPixmap::fromPmHPOINTER(hicon, &pixmap, &mini);
            if (!retIcon.isNull()) {
                // store pixmaps in the cache
                Q_ASSERT(!pixmap.isNull() || !mini.isNull());
                if (!pixmap.isNull())
                    QPixmapCache::insert(key, pixmap);
                if (!mini.isNull())
                    QPixmapCache::insert(keyMini, mini);
            }
        }
    }
...
QIcon QFileIconProvider::icon(const QFileInfo &info) const
{
...
#elif defined(Q_WS_PM)
    {
        ...
        ioc_parm[0] = 0;
        ioc_parm[1] = info.absoluteFilePath().at(0).toUpper().cell() - 'A';
        APIRET arc = DosDevIOCtl((HFILE) - 1, IOCTL_DISK, DSK_GETDEVICEPARAMS,
                                 ioc_parm, sizeof(ioc_parm), NULL,
                                 &bpb, sizeof(bpb), NULL);
