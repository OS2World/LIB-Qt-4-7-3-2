...
public:
    QSoundPrivate(const QString& fname)
        : filename(fname), server(0), bucket(0), looprem(0), looptotal(1)
    {
    }
...
/*!
    Returns true if sound \a s was initialized using this server instance
    and false otherwise.
*/
bool QAuServer::isRelevant(QSound* s)
{
    return s->d_func()->server == this;
}
