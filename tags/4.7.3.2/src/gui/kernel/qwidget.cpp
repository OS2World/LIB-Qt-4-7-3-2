...
#if defined(Q_WS_WIN) || defined(Q_WS_X11) || defined (Q_WS_QWS) || defined(Q_WS_MAC) || defined(Q_WS_PM)
...
/*!
    This special event handler can be reimplemented in a subclass to
    receive native OS/2 Presentation Manager events which are passed in
    the \a message parameter.
    ...
    \sa QApplication::pmEventFilter()
*/
bool QWidget::pmEvent(QMSG *message, MRESULT *result)
{
    Q_UNUSED(message);
    Q_UNUSED(result);
    return false;
...
void QWidget::setParent(QWidget *parent, Qt::WindowFlags f)
{
...
#if defined(Q_WS_X11) || defined(Q_WS_WIN) || defined(Q_WS_PM) || defined(Q_WS_MAC) || defined(Q_OS_SYMBIAN)
    ...
    if (!testAttribute(Qt::WA_StyleSheet)
        ...
        if (parent) {
            // take the parent's resolve masks to make sure attributes imposed
            // by the user on the parent won't be overriden by the natural
            // palette for this widget if it differs from QApplication::palette()
            QWidgetPrivate *pd = parent->d_func();
            d->inheritedFontResolveMask =
                pd->data.fnt.resolve() | pd->inheritedFontResolveMask;
            d->inheritedPaletteResolveMask =
                pd->data.pal.resolve() | pd->inheritedPaletteResolveMask;
        }
        d->resolveFont();
        d->resolvePalette();
    }
...
void QWidget::setAttribute(Qt::WidgetAttribute attribute, bool on)
{
    ...
    if (attribute == Qt::WA_PaintOnScreen && on && !inherits("QGLWidget")) {
        // see qwidget_[win|pm].cpp, ::paintEngine for details
        paintEngine();
    ...
    case Qt::WA_PaintOnScreen:
        d->updateIsOpaque();
#if defined(Q_WS_WIN) || defined(Q_WS_PM) || defined(Q_WS_X11) || defined(Q_WS_MAC) || defined(Q_OS_SYMBIAN)
...
/*!
    \fn WId QWidget::internalFrameWinId() const
    \internal
    Returns the frame window system identifier of the widget, or 0 if the widget
    is not created yet or if it is not a top-level window and therefore does not
    have a system frame.

    \warning This function is not portable and exists only on OS/2.
*/
WId QWidget::internalFrameWinId() const
{
    Q_D(const QWidget);
    return d->frameWinId();
}
...
/*!
    Returns the effective frame window system identifier of the widget, i.e. the
    native parent's frame window system identifier.

    If the widget is native, this function returns the native frame window for
    this widget (the same value as internalFrameWinId() returns). Otherwise, the
    frame window ID of the first native parent widget, i.e., the top-level
    widget that contains this widget, is returned.
    ...
    \warning This function is not portable and exists only on OS/2.
*/
WId QWidget::effectiveFrameWinId() const
{
    Q_D(const QWidget);
    return d->effectiveFrameWinId();
}
...
/*!
    ...
    An HPS acquired with getPS() has to be released with releasePS().

    \warning Using this function is not portable.
*/
HPS QWidget::getPS() const
{
    Q_D(const QWidget);
    if (d->hd != NULLHANDLE)
        return (HPS) d->hd;
#ifndef QT_NO_DRAGANDDROP
    if (QDragManager::self()->isInDrag() ||
        QDragManager::self()->isInOwnDrag())
        return DrgGetPS(winId());
#endif
    return WinGetPS(winId());
}

/*!
    Releases the HPS \a hdc acquired by a previous call to getPS().

    \warning Using this function is not portable.
*/
void QWidget::releasePS(HPS hps) const
{
    Q_D(const QWidget);
    // If there is a widget's own ps, the release operation is ignored because
    // it will be released elsewhere. Otherwise, we assume that the given hps
    // was acquired by DrgGetPS/WinGetPS in the getPS() call and release it.
    if (d->hd == NULLHANDLE) {
#ifndef QT_NO_DRAGANDDROP
        if (QDragManager::self()->isInDrag() ||
            QDragManager::self()->isInOwnDrag())
