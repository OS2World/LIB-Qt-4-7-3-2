...
public:
    ...
    WId internalFrameWinId() const;
    WId effectiveFrameWinId() const;
    HPS getPS() const;
    void releasePS(HPS) const;

    struct PmEventFilter {
        virtual bool pmEventFilter(QMSG *message, MRESULT *result) = 0;
    };
    void addPmEventFilter(PmEventFilter *filter);
    void removePmEventFilter(PmEventFilter *filter);
#else
...
#endif
#if defined(Q_WS_PM)
    virtual bool pmEvent(QMSG *message, MRESULT *result);
#endif
#if defined(Q_WS_X11)
