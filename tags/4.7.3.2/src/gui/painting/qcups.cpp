...
#ifdef Q_OS_OS2
#define CUPS_RESOLVE_cups(func) _cups##func = (Cups##func) cupsLib.resolve("_cups"#func)
#define CUPS_RESOLVE_ppd(func) _ppd##func = (PPD##func) cupsLib.resolve("_ppd"#func)
#else
#define CUPS_RESOLVE_cups(func) _cups##func = (Cups##func) cupsLib.resolve("cups"#func)
#define CUPS_RESOLVE_ppd(func) _ppd##func = (PPD##func) cupsLib.resolve("ppd"#func)
#endif
...
static void resolveCups()
{
    ...
    if(cupsLib.load()) {
        CUPS_RESOLVE_cups(GetDests);
        CUPS_RESOLVE_cups(FreeDests);
        CUPS_RESOLVE_cups(GetPPD);
        CUPS_RESOLVE_cups(LangGet);
        CUPS_RESOLVE_cups(LangEncoding);
        CUPS_RESOLVE_ppd(OpenFile);
        CUPS_RESOLVE_ppd(MarkDefaults);
        CUPS_RESOLVE_ppd(Close);
        CUPS_RESOLVE_cups(MarkOptions);
        CUPS_RESOLVE_ppd(MarkOption);
        CUPS_RESOLVE_cups(FreeOptions);
        CUPS_RESOLVE_cups(SetDests);
        CUPS_RESOLVE_cups(AddOption);
        CUPS_RESOLVE_cups(TempFd);
        CUPS_RESOLVE_cups(PrintFile);
