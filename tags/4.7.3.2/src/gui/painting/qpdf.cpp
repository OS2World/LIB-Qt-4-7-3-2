...
#if defined(Q_WS_PM)
        // some CUPS versions don't set the mode of the returned fd to O_BINARY
        // which is vital for correct PDF generation (consider binary blocks);
        // force it here
        ::setmode(ret.first, O_BINARY);
#endif
        cupsTempFile = ret.second;
        outDevice = new QFile();
        static_cast<QFile *>(outDevice)->open(ret.first, QIODevice::WriteOnly);
        // save fd to get the temporary file closed in closePrintDevice()
        fd = ret.first;
#endif
#ifndef QT_NO_LPR
    } else {
