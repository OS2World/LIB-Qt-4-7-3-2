...
#if (defined (Q_OS_UNIX) && ! defined (Q_WS_MAC)) || defined (Q_WS_PM)
...
#elif defined (Q_OS_UNIX) || defined (Q_WS_PM)
        Q_ASSERT(false);
#endif
        }
        break;
...
/*!
  ...
  to use instead of the system default. On OS/2, QPrinter also uses CUPS
  but it does not provide support for the \l lpr utility.
...
#if !defined(QT_NO_CUPS) && (!defined(QT_NO_LIBRARY) || defined(Q_WS_PM))
    // Note: On OS/2, sending PDF data to eCUPS only works starting from
    // version 1.4.4 of CUPS
#if defined(Q_WS_PM)
    if (QCUPSSupport::cupsVersion() >= 10404 && QCUPSSupport().currentPPD()) {
#else
...
#if (defined(Q_OS_UNIX) || defined(Q_WS_PM)) && !defined(QT_NO_CUPS)
    if(d->use_default_engine
        && d->outputFormat == QPrinter::NativeFormat) {
        // Note: On OS/2, sending PDF data to eCUPS only works starting from
        // version 1.4.4 of CUPS
#if defined(Q_WS_PM)
        if (QCUPSSupport::cupsVersion() >= 10404
#else
