...
public:
    ...
    inline HRGN handle(int targetHeight) const {
        if (d->rgn == NULLHANDLE || d->height != targetHeight)
            updateHandle(targetHeight);
        return d->rgn;
    }
...
#elif defined(Q_WS_PM)
    void updateHandle(int targetHeight) const;
    static void disposeHandle(HRGN hrgn);
#elif defined(Q_WS_X11)
