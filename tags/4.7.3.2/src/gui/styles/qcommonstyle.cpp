...
int QCommonStyle::pixelMetric(PixelMetric m, const QStyleOption *opt, const QWidget *widget) const
{
    ...
    case PM_SmallIconSize:
        if (QApplication::desktopSettingsAware())
            ret = WinQuerySysValue(HWND_DESKTOP, SV_CXICON) / 2;
        else
            ret = int(QStyleHelper::dpiScaled(16.));
        break;
#else
...
#ifdef Q_WS_PM
        if (QApplication::desktopSettingsAware())
            ret = WinQuerySysValue(HWND_DESKTOP, SV_CXICON);
        else
            ret = int(QStyleHelper::dpiScaled(32.));
#else
        ret = int(QStyleHelper::dpiScaled(32.));
                                               ...
                                               const QWidget *widget) const
{
...
#if defined(Q_WS_PM)
    if (QApplication::desktopSettingsAware()) {
        // Take file/folder icons from QFileProvider as it knows how to get
        // them from the system theme (see QFileProvider::pmIcon()).
        switch (standardIcon) {
        case SP_FileIcon:
        case SP_FileLinkIcon: {
            QFileIconProvider fip;
            static char kernelFilePath[] = "?:\\OS2KRNL";
            if (kernelFilePath[0] == '?') {
                ULONG bootDrive = 0;
                DosQuerySysInfo(QSV_BOOT_DRIVE, QSV_BOOT_DRIVE,
                                (PVOID)&bootDrive, sizeof(bootDrive));
                kernelFilePath[0] = bootDrive + 'A' - 1;
            }
            return fip.icon(QFileInfo(QLatin1String(kernelFilePath)));
        }
        case SP_DirIcon:
        case SP_DirLinkIcon:
        case SP_DirClosedIcon:
        case SP_DirOpenIcon: {
            // TODO: Use WPS to get icons for open folders
            // (see see QFileProvider::pmIcon() again)
            QFileIconProvider fip;
            return fip.icon(QFileInfo(QLatin1String("\\")));
        }
        default:
            break;
        }
   ...
   case SP_DirIcon:
    ...
    case SP_MediaVolumeMuted:
        ...
        break;
    case SP_ComputerIcon:
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/computer-16.png"), QSize(16, 16));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/computer-32.png"), QSize(32, 32));
        break;
    case SP_DesktopIcon:
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/desktop-16.png"), QSize(16, 16));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/desktop-32.png"), QSize(32, 32));
        break;
    case SP_DriveFDIcon:
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/floppy-16.png"), QSize(16, 16));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/floppy-32.png"), QSize(32, 32));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/floppy-128.png"), QSize(128, 128));
        break;
    case SP_DriveHDIcon:
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/harddrive-16.png"), QSize(16, 16));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/harddrive-32.png"), QSize(32, 32));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/harddrive-128.png"), QSize(128, 128));
        break;
    case SP_DriveNetIcon:
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/networkdrive-16.png"), QSize(16, 16));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/networkdrive-32.png"), QSize(32, 32));
        icon.addFile(QLatin1String(":/trolltech/styles/commonstyle/images/networkdrive-128.png"), QSize(128, 128));
        break;
#endif // QT_NO_IMAGEFORMAT_PNG
    default:
...
#if defined(Q_WS_PM)
    if (QApplication::desktopSettingsAware() && !icon.isNull()) {
        // make sure the file dialog related icons have a size not smaller than
        // the system icon size by centering them (this makes sure the
        // corresponding items are aligned with file items with system icons)
        switch (standardIcon) {
        case SP_DriveCDIcon:
        case SP_DriveDVDIcon:
        case SP_DriveNetIcon:
        ...
        case SP_DirHomeIcon:
        case SP_TrashIcon: {
            static QIcon::Mode modes[] = { QIcon::Normal, QIcon::Active,
            ...
            for (size_t m = 0; m < sizeof(modes)/sizeof(modes[0]); ++m) {
                for (size_t s = 0; s < sizeof(states)/sizeof(states[0]); ++s) {
                    ...
                    QIcon::State ss = states[s];
                    QList<QSize> sizes = icon.availableSizes(mm, ss);
                    QSize smallSz, largeSz;
                    foreach(const QSize &asz, sizes) {
                        if (asz.width() < small && asz.width() > smallSz.width())
                            smallSz = asz;
                        else if (asz.width() < large && asz.width() > largeSz.width())
                            largeSz = asz;
                    }
                    if (smallSz.isValid() && !largeSz.isValid()) {
                        // add the large icon if missing by up-scaling the small one by 2
                        QPixmap p = icon.pixmap(smallSz, mm, ss);
                        largeSz = smallSz * 2;
                        icon.addPixmap(p.scaled(largeSz, Qt::KeepAspectRatio,
                                                Qt::SmoothTransformation));
                    } else if (largeSz.isValid() && !smallSz.isValid()) {
                        // add the small icon if missing by down-scaling the large one by 2
                        QPixmap p = icon.pixmap(largeSz, mm, ss);
                        largeSz = largeSz / 2;
                        icon.addPixmap(p.scaled(smallSz, Qt::KeepAspectRatio,
                                                Qt::SmoothTransformation));
                    }
                    if (smallSz.isValid()) {
                        QPixmap pixmap = icon.pixmap(smallSz, mm, ss);
                        QPixmap centered(small, small);
                        centered.fill(Qt::transparent);
                        QPainter painter(&centered);
                        int x = (small - pixmap.width()) / 2;
                        int y = (small - pixmap.height()) / 2;
                        painter.drawPixmap(x, y, pixmap);
                        painter.end();
                        icon.addPixmap(centered, mm, ss);
                    }
                    if (largeSz.isValid()) {
                        QPixmap pixmap = icon.pixmap(largeSz, mm, ss);
                        QPixmap centered(large, large);
                        centered.fill(Qt::transparent);
                        QPainter painter(&centered);
                        int x = (large - pixmap.width()) / 2;
                        int y = (large - pixmap.height()) / 2;
                        painter.drawPixmap(x, y, pixmap);
                        painter.end();
                        icon.addPixmap(centered, mm, ss);
                    }
