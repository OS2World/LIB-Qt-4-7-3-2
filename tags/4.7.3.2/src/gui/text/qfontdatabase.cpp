...
#endif
#if defined(Q_WS_QWS) || defined(Q_OS_SYMBIAN) || defined(Q_WS_PM)
    QByteArray fileName;
    int fileIndex;
...
#endif
#ifdef Q_WS_PM
            pixelSizes[count].systems.~QList<QFontDatabase::WritingSystem>();
#endif
#if defined(Q_WS_QWS) || defined(Q_OS_SYMBIAN) || defined(Q_WS_PM)
...
#endif
#ifdef Q_WS_PM
    new (&pixelSizes[count].systems) QList<QFontDatabase::WritingSystem>;
#endif
#if defined(Q_WS_QWS) || defined(Q_OS_SYMBIAN) || defined(Q_WS_PM)
...
#if ((defined(Q_WS_QWS) || defined(Q_WS_PM)) && !defined(QT_NO_FREETYPE)) || defined(Q_WS_WIN)  || defined(Q_OS_SYMBIAN) || (defined(Q_WS_MAC) && !defined(QT_MAC_USE_COCOA))
...
#if defined(Q_WS_X11) || defined(Q_WS_WIN) || defined(Q_OS_SYMBIAN) || defined(Q_WS_PM)
...
#if defined(Q_WS_PM)
static bool sizeSupportsScript(int script, QtFontSize *size)
{
    // empty writing system list means we support all
    // (see qfontdatabase_pm.cpp for explanation)
    if (size->systems.isEmpty())
        return true;
    foreach (QFontDatabase::WritingSystem ws, size->systems) {
        if (scriptForWritingSystem[ws] == script)
            return true;
    }
    return false;
...
#endif
#ifdef Q_WS_PM
                if (!sizeSupportsScript(script, style->pixelSizes + x)) {
                    FM_DEBUG("          size %3d does not support the script we want",
                             style->pixelSizes[x].pixelSize);
                    continue;
                }
...
#ifdef Q_WS_PM
        if (size) {
            if (!sizeSupportsScript(script, size)) {
                size = 0;
                FM_DEBUG("          foundry doesn't support the script we want");
                continue;
            }
