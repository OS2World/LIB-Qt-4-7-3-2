...
void QBalloonTip::balloon(const QPoint& pos, int msecs, bool showArrow)
{
    ...
    QSize sh = sizeHint();
    const int border = 1;
    const int ah = 18, ao = 18;
#if !((defined(QT_NO_XSHAPE) && defined(Q_WS_X11)) || defined(Q_WS_PM))
    const int aw = 18, rc = 7;
#endif
    ...
    // draw an ugly square when its not available. Also the case on OS/2
    path.moveTo(0, 0);
    ...
    if (arrowAtTop && arrowAtLeft) {
        move(qMax(pos.x(), scr.left() + 2), pos.y());
    } else if (arrowAtTop && !arrowAtLeft) {
        move(qMin(pos.x() - sh.width(), scr.right() - sh.width() - 2), pos.y());
    } else if (!arrowAtTop && !arrowAtLeft) {
        move(qMin(pos.x() - sh.width(), scr.right() - sh.width() - 2), pos.y() - sh.height());
    } else if (!arrowAtTop && arrowAtLeft) {
        move(qMax(pos.x(), scr.x() + 2), pos.y() - sh.height());
    }
#else
