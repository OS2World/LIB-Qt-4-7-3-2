...
void QLocalSocket::connectToServer(const QString &name, OpenMode openMode)
{
    ...
    // create the socket
    if (-1 == (d->connectingSocket = ::socket(PF_UNIX, SOCK_STREAM, 0))) {
...
void QLocalSocketPrivate::_q_connectToSocket()
{
    ...
    name.sun_len = sizeof(sockaddr_un);
    QByteArray cpn = connectingPathName.toLocal8Bit();
    if (sizeof(name.sun_path) < (uint)cpn.size() + 1) {
        QString function = QLatin1String("QLocalSocket::connectToServer");
        errorOccurred(QLocalSocket::ServerNotFoundError, function);
        return;
    }
    ::strcpy(name.sun_path, cpn);

    if (-1 == ::connect(connectingSocket, (struct sockaddr *)&name, sizeof(name))) {
        QString function = QLatin1String("QLocalSocket::connectToServer");
        switch (errno)
        {
        case EINVAL:
