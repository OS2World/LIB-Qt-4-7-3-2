...
QList<QSslCertificate> QSslSocketPrivate::systemCaCertificates()
{
...
#elif (defined(Q_OS_UNIX) && !defined(Q_OS_SYMBIAN)) || defined(Q_OS_OS2)
...
#else
    directories << "/@unixroot/etc/ssl/certs/";
    QByteArray etc = qgetenv("ETC");
    if (!etc.isEmpty())
        directories << etc.append("/ssl/certs/");
#endif
    ...
    for (int a = 0; a < directories.count(); a++) {
        currentDir.setPath(QFile::decodeName(directories.at(a)));
