...
void StdInListenerWin::run()
{
    ...
    HFILE hStdinDup = HFILE(~0);
    APIRET arc = DosDupHandle(HFILE(0), &hStdinDup);
    Q_ASSERT(arc == NO_ERROR);

    while (arc == NO_ERROR) {
        arc = DosRead(hStdinDup, chBuf, sizeof(chBuf), &ulRead);
        if (arc == NO_ERROR && ulRead != 0) {
            // we may get data in chunks, behave like receivedData()
            ULONG i = 0;
            bool gotIt = false;
            while (!gotIt && i < ulRead) {
                if (chBuf[i] == EOF || chBuf[i] == '\0')
                    gotIt = true;
                else if (chBuf[i++] == '\n')
                    gotIt = true;
            }
            cmd.append(chBuf, i);
            if (gotIt) {
                emit receivedCommand(QString::fromLocal8Bit(cmd));
                cmd.clear();
                // get the remains if any
                cmd.append(chBuf + i, ulRead - i);
            }
        }
    }
...
#endif // defined(Q_OS_OS2)

#endif // defined(Q_OS_WIN) || defined(Q_OS_OS2)
